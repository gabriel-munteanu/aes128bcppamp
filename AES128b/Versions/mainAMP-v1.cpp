...
struct Constants{
	const int Logtable[256];
	const int Alogtable[256];
	const int S[256];
	const int Si[256];
	const int Key[11][4][4];
};


int mul(int a, int b, const int alogtable[246], const int logtable[256]) restrict(amp)
...
void AddRoundKey(int data[4][4], const int rk[4][4]) restrict(amp)
{
...
void SubBytes(int data[4][4], const int box[256]) restrict(amp)
{
...
void ShiftRows(int a[4][4], int d) restrict(amp)
{
...
void MixColumns(int a[4][4], const int alogtable[246], const int logtable[256]) restrict(amp)
{
...
void InvMixColumns(int a[4][4], const int alogtable[246], const int logtable[256]) restrict(amp)
{
	/* Mix the four bytes of every column in a linear way
...
void Encrypt(int data[4][4], Constants constants) restrict(amp)
{
	/* Encryption of one block. */
	int r;
	/* begin with a key addition */
	AddRoundKey(data, constants.Key[0]);
	/* ROUNDS-1 ordinary rounds */
	for (r = 1; r < 10; r++)
	{
		SubBytes(data, constants.S);
		ShiftRows(data, 0);
		MixColumns(data, constants.Alogtable, constants.Logtable);
		AddRoundKey(data, constants.Key[r]);
	}
	/* Last round is special : there is no MixColumns */
	SubBytes(data, constants.S);
	ShiftRows(data, 0);
	AddRoundKey(data, constants.Key[10]);
}

void Decrypt(int data[4][4], Constants constants) restrict(amp)
{
	int r;
	/* To decrypt :
	...
	* with extra AddRoundKey */
	AddRoundKey(data, constants.Key[10]);
	SubBytes(data, constants.Si); ShiftRows(data, 1);
	/* RDUNDS-1 ordinary rounds */

	for (r = 10 - 1; r > 0; r--)
	{
		AddRoundKey(data, constants.Key[r]);
		InvMixColumns(data, constants.Alogtable, constants.Logtable);
		SubBytes(data, constants.Si);
		ShiftRows(data, 1);
	}
...
void AES128Amp(unsigned char *data, long size, unsigned char key[11][4][4], bool encrypt)
{
	...
	for (int i = 0; i < 11 * 16; i++)
		((int*)constValues.Key)[i] = (int)((char*)key)[i];

	int iEncrypt = encrypt ? 1 : 0;
	...
	parallel_for_each(d_ext, [=](index<1> idx) restrict(amp)
	{
		...
		int matr[4][4];
		//first line expanded
		/*matr[0][0] = d_data[d_pos] >> 24;
		matr[0][1] = (d_data[d_pos] >> 16) & 0xFF;
		matr[0][2] = (d_data[d_pos] >> 8) & 0xFF;
		matr[0][3] = (d_data[d_pos] >> 0) & 0xFF;*/
		for (int i = 0; i < 4; i++)
			for (int s = 24, j = 0; s >= 0; s -= 8, j++)
				matr[i][j] = (d_data[d_pos + i] >> s) & 0xFF;

		//do all your calculations

		Encrypt(matr, constValues);


		//write back the matrix to its linear form
		for (int i = 0; i < 4; i++)
			d_data[d_pos + i] = (matr[i][0] << 24) | (matr[i][1] << 16) | (matr[i][2] << 8) | matr[i][3];
