...
void AES128CPU::SequentialEncryption() {

	for (unsigned char *i = _data; i - _data < _dataLength; i += 16) {	
		EncryptBlock((word8(*)[4])i);
	}
}


void AES128CPU::SequentialDecryption() {

	for (unsigned char *i = _data; i - _data < _dataLength; i += 16) {	
		DecryptBlock((word8(*)[4])i);
	}
}
